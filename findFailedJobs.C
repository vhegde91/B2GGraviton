...
void findFailedJobs(string arg){
  ...
  for(int i=0;;i++){
    ...
    errMessage = name1;
    errMessage = "grep ERROR "+errMessage;
    errMessage.ReplaceAll(".jdl",".stderr");
    hasError = !(system(errMessage));
    if(!hasError) {
      errMessage = name1;
      errMessage = "grep zzzz "+errMessage;
      errMessage.ReplaceAll(".jdl",".stdout");
      hasError = (system(errMessage));
    }
